...
{	
    ...
    UNROLL_LOOP for (i = 0; i < AOIT_RT_COUNT; ++i) {
	    UNROLL_LOOP for (j = 0; j < 4; ++j) {
    ...
    UNROLL_LOOP for (i = AOIT_NODE_COUNT - 1; i >= 0; --i) {
        [flatten]if (index <= i) {
            depth[i + 1] = depth[i];
    ...
    // Insert new fragment
    UNROLL_LOOP for (i = 0; i <= AOIT_NODE_COUNT; ++i) {
        [flatten]if (index == i) {
    ...
    [flatten]if (depth[AOIT_NODE_COUNT] != AIOT_EMPTY_NODE_DEPTH) {	                
        ...
        UNROLL_LOOP for (i = startRemovalIdx; i < removalCandidateCount; ++i) {
        ...
        UNROLL_LOOP for ( ; i < removalCandidateCount; ++i) {
        ...
        // Remove that node..
        UNROLL_LOOP for (i = startRemovalIdx; i < AOIT_NODE_COUNT; ++i) {
            [flatten]if (smallestErrorIdx <= i) {
                depth[i] = depth[i + 1];
            }
        }
        UNROLL_LOOP for (i = startRemovalIdx - 1; i < AOIT_NODE_COUNT; ++i) {
